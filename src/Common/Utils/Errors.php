...
namespace WoogaEnhancedEcommerce\Common\Utils;
...
class Errors {
	...
	public static function wpDie( $message = '', $subtitle = '', $source = '', $exception = '', $title = '' ) {
		if ( $message ) {
			...
			$title = $title ? $title : $plugin['name'] . ' ' . $plugin['version'] . ' ' . __( '&rsaquo; Fatal Error', 'wooga-enhanced-ecommerce' );
			Errors::writeLog(
				[
			...
			$source = $source ? '<code>' .
				sprintf(  /* translators: %s: file path */
					__( 'Error source: %s', 'wooga-enhanced-ecommerce' ), $source
				) . '</code><BR><BR>' : '';
	...
	public static function pluginDie( $message = '', $subtitle = '', $source = '', $exception = '', $title = '' ) {
		if ( $message ) {
			$plugin = self::getPluginData();
			$title = $title ? $title : $plugin['name'] . ' ' . $plugin['version'] . ' ' . __( '&rsaquo; Plugin Disabled', 'wooga-enhanced-ecommerce' );
			Errors::writeLog(
				[
					'title'     => $title . ' - ' . $subtitle,
					'message'   => $message,
			...
			global $wooga_enhanced_ecommerce_die_notice;
			$wooga_enhanced_ecommerce_die_notice = $error;
				...
				static function () {
					...
					echo wp_kses_post(
						sprintf(
							...
							$wooga_enhanced_ecommerce_die_notice
						)
					);
				}
			);
		...
		add_action( 'admin_init',
			static function () {
				deactivate_plugins( plugin_basename( WOOGA_ENHANCED_ECOMMERCE_PLUGIN_FILE ) ); // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
			}
		);
	}
